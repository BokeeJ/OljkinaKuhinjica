...
export default function ReceptDetalji() {
  ...
  const catDisp = useMemo(
    () => prettyCategory(recipe?.category || ""),
    [recipe?.category]
  );

  const showSection = sectionDisp && !eqi(sectionDisp, catDisp);
  const showSub = subDispList.length > 0;

  /* ✅ NAPOMENA + POSLOVICA (opciono) */
                    ...
                    item.kind === "video" ? (
                      ...
                      <img
                        ...
                        onClick={() => {
                          ...
                          setCurrentIndex(Math.max(0, imageIndex));
                          setIsOpen(true);
                        }}
                      />
                    )
