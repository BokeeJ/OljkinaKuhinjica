...
export default function SviRecepti() {
  ...
  useEffect(() => {
    ...
    // ⬇️ šaljemo RAW vrednosti, bez canon()
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-white to-emerald-50">
      <div className="mx-auto w-full max-w-7xl px-3 sm:px-4 md:px-6 py-6 space-y-6">
        ...
        <div className="rounded-3xl border border-zinc-200 bg-white/70 backdrop-blur p-3 md:p-4 shadow-sm">
          <RecipeFilterClick onPick={handleFilterPick} onReset={resetFilters} current={{ category, section, subcategory }} />
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          ...
          {!loading &&
            items.map((r) => {
              ...
              // ✅ NOVO: sections[] -> prikaz + fallback na legacy section
              const sectionsArr = Array.isArray(r.sections) ? r.sections.filter(Boolean) : [];
              const activeSection = sectionsArr[0] || r.section || "";

              // za badge sekcije: prikaži sve iz sections ako postoje, inače legacy section
              const secRaw = sectionsArr.length ? sectionsArr.join(", ") : (r.section || "");
              const secDisp = prettySection(secRaw);

              // ✅ NOVO: subcategories[] -> fallback legacy subcategory
              const subRaw =
                Array.isArray(r.subcategories) && r.subcategories.length
                  ? r.subcategories.join(", ")
                  : (r.subcategory || "");

              // displaySub zavisi od aktivne sekcije (prve)
              const subDisp = displaySub(activeSection, subRaw);

              // sakrij badge ako bi bio duplikat (case-insensitive)
              const showSection =
                secDisp && !eqi(secDisp, catDisp) && !eqi(secDisp, subDisp);
              const showSub =
                subDisp && !eqi(subDisp, secDisp) && !eqi(subDisp, catDisp);
                  ...
                  <div className="px-3 pb-3 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      <button
                        ...
                        className={`px-2 py-1 rounded-full text-[11px] shadow-sm ${
                          likedSet.has(r._id)
